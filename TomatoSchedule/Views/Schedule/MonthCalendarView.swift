...
struct MonthCalendarView: View {
    ...
    private var displayMonth: Date { DateHelper.calendar.startOfDay(for: focusDate) }
    ...
    private func buildSnapshot() -> MonthSnapshot {
        ...
        let monthLessons = lessons.filter { $0.date >= range.start && $0.date < range.end }
        ...
        for (key, dayLessons) in grouped {
            blocks[key] = dayLessons.map { lesson in
