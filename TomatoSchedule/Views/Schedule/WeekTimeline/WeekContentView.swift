...
///
/// Time-axis labels are NOT here — they live in the sticky TimeAxisColumn at the
/// WeekTimelineView level so horizontal week swipes do not slide the labels.
///
/// Performance notes:
/// - No GeometryReader inside the body — the parent (WeekTimelineView) measures
///   the page width once and hands it in via `weekWidth`. GeometryReader was
///   causing an extra layout pass per page, multiplied across visible weeks.
struct WeekContentView: View {
    ...
    private var columnWidth: CGFloat { max((weekWidth - 48) / 7, 0) }
    ...
    var body: some View {
        ScrollViewReader { reader in
            ScrollView(.vertical, showsIndicators: true) {
                ...
                .frame(width: weekWidth, height: contentHeight, alignment: .top)
    ...
    private var dayColumnsLayer: some View {
        HStack(spacing: 0) {
